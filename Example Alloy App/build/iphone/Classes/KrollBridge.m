...
NSString * GoogleAuthAlloy$ModuleRequireFormat = @"(function(exports){"
...
@implementation GoogleAuthAlloyObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_googleauthalloy);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create GoogleAuthAlloy global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "GoogleAuthAlloy" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_googleauthalloy = [[GoogleAuthAlloyObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_googleauthalloy];
	
	NSString *_googleauthalloyNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _googleauthalloyNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_googleauthalloy valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:GoogleAuthAlloy$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.googleauthalloy.kroll" 
