...
//Rare exceptions to indicate a bug in the _googleauthalloy code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_googleauthalloy = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_googleauthalloy]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.googleauthalloy.module"
