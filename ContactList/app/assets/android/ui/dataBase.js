function dataBase(request,data){
  ...
  switch(request){
    case 'create':
      ...
      name TEXT,cel INTEGER,email TEXT,address TEXT,imgUrl TEXT,
    ...
    case 'add':
      ...
      db.execute('INSERT INTO contacts(name,cel,email,address,imgUrl,
        ...
        data.email,data.address,data.url,data.latitude,data.longitude);
      db.close();
      alert("added");
      break;
    case 'querying':
    ...
    case 'query':
      ...
      while(allData.isValidRow()){
        data1.push(allData.fieldByName('imgUrl'));
